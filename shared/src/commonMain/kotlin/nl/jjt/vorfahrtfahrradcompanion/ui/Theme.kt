 ...
 * dark and saturated against a near-white background, because this is what has to be read in
 * sunlight from a holder on the handlebars. Every colour that carries text or an edge clears 7:1
 * against what it sits on, well past the 4.5:1 that counts as legible indoors: a phone at half
 * brightness under a bright sky loses most of the difference between two mid-tones.
 */
...
fun Spotlight(lit: Boolean, content: @Composable () -> Unit) = MaterialTheme(
    colorScheme = if (lit && !LocalNight.current) LightColors else MaterialTheme.colorScheme,
    content = content,
)
