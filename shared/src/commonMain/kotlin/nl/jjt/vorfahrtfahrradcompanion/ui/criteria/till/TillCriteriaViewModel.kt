...
sealed interface TillCriteriaUiState {
    data object Loading : TillCriteriaUiState
    data class Failed(val message: String) : TillCriteriaUiState
    data class Ready(
...
class TillCriteriaViewModel(
    private val api: CriteriaApi,
    private val segments: SegmentRecorder,
    private val rides: RideRecorder,
) : ViewModel() {

    private val _state = MutableStateFlow<TillCriteriaUiState>(TillCriteriaUiState.Loading)
    val state: StateFlow<TillCriteriaUiState> = _state.asStateFlow()
    ...
    /** Reloads the catalogue; reachable from [TillCriteriaUiState.Failed], where no cached copy was available. */
    fun retry() = load()
     ...
     * entered anything, which is when [TillCriteriaUiState.Ready.copyable] stops offering it.
     */
    fun copyPrevious() {
        val values = (_state.value as? TillCriteriaUiState.Ready)?.copyable ?: return
     ...
     * stretch is worth keeping at all, so it is asked as a [TillCriteriaUiState.Ready.pendingTiming] before
    ...
    fun answerTiming(timing: EndTiming) {
        val request = (_state.value as? TillCriteriaUiState.Ready)?.pendingTiming ?: return
    ...
    private fun canEnd(): Boolean {
        val ready = _state.value as? TillCriteriaUiState.Ready ?: return false
    ...
    private fun ask(request: EndRequest) {
        val ready = _state.value as? TillCriteriaUiState.Ready ?: return
    ...
    fun confirmEnd(approve: Set<Criterion>) {
        val request = (_state.value as? TillCriteriaUiState.Ready)?.pendingEnd ?: return
    ...
    private fun load() {
        _state.value = TillCriteriaUiState.Loading
        viewModelScope.launch {
            _state.value = try {
                val draft = segments.draft.value
                TillCriteriaUiState.Ready(
                ...
                TillCriteriaUiState.Failed(e.message ?: "Could not load the criterion catalogue")
            }
        }
    }
    ...
    private fun updateReady(edit: TillCriteriaUiState.Ready.() -> TillCriteriaUiState.Ready) {
        _state.update { if (it is TillCriteriaUiState.Ready) it.edit() else it }
    }
}
