package nl.jjt.vorfahrtfahrradcompanion.ui.criteria.till
...
class TillCriteriaViewModelTest {
    ...
    private fun vm(): TillCriteriaViewModel {
        val rides = RideRecorder(rideStore, observations, clock)
        return TillCriteriaViewModel(FakeCriteriaApi(catalogue), SegmentRecorder(observations, rides, clock), rides)
    }

    /** A view model with a ride already running — the only state in which segments can be recorded. */
    private fun TestScope.riding(): TillCriteriaViewModel {
        val vm = vm()
        vm.startRide()
    ...
    fun endingWithoutAStartIsIgnored() = runTest {
        ...
        assertEquals(Segment.Idle, (vm.state.value as TillCriteriaUiState.Ready).segment)
    }
    ...
    fun startingTwiceKeepsTheFirstBoundary() = runTest {
        ...
        val segment = (vm.state.value as TillCriteriaUiState.Ready).segment
    ...
    fun theFirstTapOnACarriedOverValueApprovesItRatherThanClearingIt() = runTest {
        ...
        val ready = vm.state.value as TillCriteriaUiState.Ready
        ...
        vm.onTap(users, cars)
        testScheduler.advanceUntilIdle()
        assertEquals(emptySet(), (vm.state.value as TillCriteriaUiState.Ready).answers[users])
    }
    ...
    fun answersCarryOverIntoTheSegmentThatContinuesFromTheSameBoundary() = runTest {
        ...
        val state = vm.state.value as TillCriteriaUiState.Ready
    ...
    fun thereIsNothingToCopyUntilSomethingHasBeenSubmitted() = runTest {
        ...
        assertNull((vm.state.value as TillCriteriaUiState.Ready).copyable)
    }
    ...
    fun aFreshSegmentCanBeFilledInFromTheLastOneSubmitted() = runTest {
        ...
        assertEquals(
            ...
            (vm.state.value as TillCriteriaUiState.Ready).copyable,
        )
    ...
    fun pressingEndAsksHowWellItWasCaughtBeforeStoringAnything() = runTest {
        ...
        assertEquals(emptyList(), observations.inserted)
        assertEquals(SegmentAction.STOP, (vm.state.value as TillCriteriaUiState.Ready).pendingTiming?.action)

        vm.answerTiming(EndTiming.PRECISE)
        testScheduler.advanceUntilIdle()

        assertNull((vm.state.value as TillCriteriaUiState.Ready).pendingTiming)
        assertEquals(BoundaryKind.EXACT, stored.endKind)
    }
    ...
    fun aLateEndStillAsksAboutWhatIsCarriedOver() = runTest {
        ...
        // survives it.
        assertNotNull((vm.state.value as TillCriteriaUiState.Ready).pendingEnd)
    ...
    fun theAnswerKeepsWhatWasApprovedAndDropsTheRest() = runTest {
        ...
        assertNull((vm.state.value as TillCriteriaUiState.Ready).pendingEnd)
    }
    ...
    fun anAnswerChangedOnTheWayOutIsStoredAsChanged() = runTest {
        ...
        assertEquals(listOf(width, users), (vm.state.value as TillCriteriaUiState.Ready).pendingEnd?.asked)
    ...
    fun theFlowMovesDownTheListThenComesBackForWhatWasSkipped() {
        ...
        val state = TillCriteriaUiState.Ready(
    ...
    private fun TestScope.aSegmentThenTheNext(vm: TillCriteriaViewModel) {
        vm.onTap(users, cars)
        vm.start(BoundaryKind.EXACT)
        clock += stretch
    ...
    fun aRideWithoutSegmentsIsThrownAwayRatherThanStored() = runTest {
        ...
        assertEquals(Ride.Idle, (vm.state.value as TillCriteriaUiState.Ready).ride)
    }
    ...
    fun leavingTheSummaryLetsTheRideRunOn() = runTest {
        ...
        assertNull(vm.endingRide.value)
        assertNull(storedRide.endedAt)
        assertTrue((vm.state.value as TillCriteriaUiState.Ready).ride is Ride.Open)
    }

    private fun TestScope.recordASegment(vm: TillCriteriaViewModel) {
        vm.onTap(users, cars)
        vm.start(BoundaryKind.EXACT)
        clock += stretch
        vm.endAs(SegmentAction.STOP, EndTiming.PRECISE)
     ...
     */
    private fun TestScope.aSubmittedSegmentThenAFreshOne(vm: TillCriteriaViewModel) {
        vm.onTap(width, w1)
        vm.onTap(users, cars)
        vm.start(BoundaryKind.EXACT)
        clock += stretch
    ...
    private fun TestScope.bothCarriedOverIntoTheNextSegment(vm: TillCriteriaViewModel) {
        vm.onTap(width, w1)
        vm.onTap(users, cars)
        vm.start(BoundaryKind.EXACT)
        clock += stretch
