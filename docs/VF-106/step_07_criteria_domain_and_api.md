...
- `CriteriaDto.kt` — `@Serializable` DTOs + `toDomain()`. Ignores `weightRange`,
  `importanceRange`, `defaultImportance`. **Unknown `kind` strings degrade to `MULTI`** rather than failing the parse.
- `CriteriaApi.kt` — `interface CriteriaApi { suspend fun catalogue(): Catalogue; suspend fun submit(o: Observation) }`
- `KtorCriteriaApi.kt` — reads baseUrl/credentials from `SettingsRepository` **per call**, so editing settings takes
  effect on the next request with no client rebuild.
...
**The POST contract is invented** — nothing in `../../http-testing` documents it. Keep the DTO and its mapping in one
small file so correcting it is a 5-line change.
