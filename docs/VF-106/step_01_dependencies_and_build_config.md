...
1. `../../gradle/libs.versions.toml` — add versions and library/plugin aliases. No version may be hardcoded in a
   `../../build.gradle.kts`.
   ...
   `androidx-room-runtime`, `androidx-room-compiler`, `androidx-sqlite-bundled`. Plugins: `kotlin.plugin.serialization`,
   `ksp`, `androidx.room`.

2. `../../shared/build.gradle.kts`
    - apply the serialization, KSP and Room plugins
    - `commonMain`: ktor core + content-negotiation + json + auth, serialization-json, room-runtime, sqlite-bundled
...
KSP 2.3.x runs on its own version line, decoupled from Kotlin. 2.3.9 is unverified against Kotlin 2.4.10. If the build
fails, fall back to the newest `2.4.10-*` KSP release.
